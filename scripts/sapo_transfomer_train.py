...
try:
    # back-compat: some versions name it RewardWeights, others EnvWeights
    from environment_differentiable_batched import RewardWeights  # type: ignore
except Exception:
    from environment_differentiable_batched import EnvWeights as RewardWeights  # type: ignore
from sapo_transofomer_agent import SAPOAgentBatched, SAPOConfig
...
def evaluate_policy(
    ...
    *,
    ...
    Uses the same (history tokens + preview features) interface as training.
    Returns mean return per env and some simple stats.
    """
    B = env.B

    def _get_preview(env_state) -> torch.Tensor:
        if hasattr(env, "get_kappa_preview") and hasattr(env, "get_v_preview"):
            k = env.get_kappa_preview(env_state)
            vpr = env.get_v_preview(env_state)
            return torch.cat([k, vpr], dim=1)
        # fallback: no preview available
        return torch.zeros((B, 0), device=env.device, dtype=env.dtype)

    env_state = env.get_env_state()
    obs_raw, _state_vec, _cache = env.compute_obs_state(env_state, return_cache=True)
    obs = env.normalize_obs(obs_raw)

    L_hist = int(getattr(agent.cfg, "history_len", 1))
    hist_seq = torch.zeros((B, L_hist, obs.shape[1]), device=env.device, dtype=env.dtype)
    hist_seq[:, -1, :] = obs
    preview = _get_preview(env_state)
    ...
    for _ in range(int(steps)):
        a = agent.actor.act(hist_seq, preview=preview, deterministic=True)
        obs_next, _obs_raw, _state_vec, r, done, _info = env.step(a, compute_info=False)
        ...
        env_state = env.get_env_state()
        preview = _get_preview(env_state)
        hist_seq = torch.cat([hist_seq[:, 1:, :], obs_next.unsqueeze(1)], dim=1)
...
def evaluate_policy_detailed(
    ...
    *,
    ...
    for _t in range(int(max_steps)):
        ...
        action = agent.actor.act(hist_seq, preview=preview, deterministic=True)
        ...
        # update history + preview (and clear history for reset envs)
        env_state = env.get_env_state()
        preview = _get_preview(env_state)
        hist_seq = torch.cat([hist_seq[:, 1:, :], obs.unsqueeze(1)], dim=1)
        if use_partial_reset and done_for_reset.any():
            reset_seq = torch.zeros_like(hist_seq)
            reset_seq[:, -1, :] = obs
            hist_seq = torch.where(done_for_reset.view(-1, 1, 1), reset_seq, hist_seq)
...
def main():
    ...
    parser.add_argument("--resume", type=str, default=False, help="Path to checkpoint to resume from (default: None)")
